    ...
    <form action="{{ route('artikel.update',$data->id) }}" method="POST">
        ...
        <div class="col-md-12">
            <div class="card mb-4">
                <div class="card-body">
                    <div class="row">
                        <x-input-container name="judul" placeholder="Masukkan Judul" id="judul"  value="{{ $data->judul_artikel }}" required/>
                        <div class="mb-3">
                            <textarea name="konten" class="form-control" id="konten" cols="20"  rows="5" placeholder="Isikan Konten">{{ $data->isi_artikel }}</textarea>
                        </div>
                    </div>
