...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('artikel.index') }}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
            class="fas fa-arrow-left fa-sm text-white-50"></i> Kembali</a>    </div>
                    ...
                    <form action="{{ route('artikel.update', $data->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control form-control-user @error('judul') is-invalid @enderror"
                                id="judul" name="judul" value="{{ old('judul', $data->judul_artikel) }}"
                        ...
                        <div class="form-group">
                            <textarea name="konten" id="konten" cols="30" rows="5" class="form-control @error('konten') is-invalid @enderror" placeholder="Masukkan konten disini...">{{old('konten', $data->isi_artikel)}}</textarea>
                            @error('konten')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
